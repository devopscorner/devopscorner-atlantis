...
resource "aws_eks_node_group" "devops" {
  ...
  instance_types = local.env == "prod" ? ["m5.xlarge"] : ["t3.xlarge"]
  disk_size      = 100
...
# --------------------------------------------------------------------------
## App Output #
output "eks_node_name_devops_app" {
  value = aws_eks_node_group.devops["app"].id
}
...
output "eks_node_tg_devops_app" {
  value = aws_lb_target_group.devops["app"].id
}
